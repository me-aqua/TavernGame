# main 上旧版 AGENTS.md（留档，待归位）

> ⚠️ 本分支的 `AGENTS.md` 只留纪律（24 行），其余内容早已拆进 `.agents/skills/`。
> 这份是 main 那一版**全文留档**，防止合并时丢掉它独有的段落（路线图、做卡系统状态、
> 各条实测教训、CORS 实测结果、素材版权提醒、协作约定…）。
> 归位方式：逐段判断该进哪个 skill 或 `doc/`，搬完即可删除本文件。
...
> ✅ **目前没有未完成的待办。** 下面是已完成项，留档备查。

### ✅ 已完成留档 —— README 的定位与重写（用户 2026-09-11 要求）
...
> ✅ **已完成（v0.5.4）**：README 已按上述原则重写，**净 −47 行**（删 55 / 加 8，124 行 → 77 行）。
>
> ⚠️ 上面表里的**行号是重写前那份 README 的**（对着 `dd56bfb^:README.md` 逐条核过，5 处全中）。
> 现在的 README 只有 77 行，照着行号去翻会翻到别的内容 —— 这段留着当**历史记录**，不是待办。
...
| 5 | `index.html` | `URL.revokeObjectURL` 紧跟 `a.click()` —— Chromium 正常，但部分浏览器可能在下载启动前释放 blob | 惯例是放进 `setTimeout`；**不确定**是否真会出问题 |
...
- **文档里关于「内容」的正文，不该由代码硬编码。**
  ...
  > 已经顺手把「五块」「N 个节点」「N 节点」这类数字都改成从 JSON 数了
  > （`CN[Object.keys(c.给AI).length]` / `CN[c.节点.length]` / `c.拓扑.length`）。
  > 圈码也补到了 ⑳ —— 原来只写到 ⑧，第 10 个节点会渲染成 `undefined`。
