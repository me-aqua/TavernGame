...
- **显示 = 真值的投影**（doc/DESIGN.md 决定 #22）：故事区不再是一个「有人 push、
  没人负责寿命」的数组，而是 `data.log` 的 computed 投影 ——
  刷新不需要「恢复」，也不可能再出现「写了却没人删」的行
- 三类内容各有各的家：
  - **进行中**（正在生成开场 / 思考中）→ `phase` 算出来的状态行，回合结束即消失
  - **通知**（导出完成 / 保存失败 / 继续游戏…）→ **单槽** notice，后一条覆盖前一条，
    回合开始清空；不再写进故事
  - **agent 信息**（模型输入输出、工具调用与结果）→ 只进调试痕迹，**调试模式之外不产生**：
    玩家不再在故事里看到 `⚙ 调用 advance_time` 与 `⚠ 模型这一步只调用了工具`
- 调试痕迹**刷新后还在**（单独的键 `tavernGame.trace`，上限 100 条，换一局清空）：
  它不进存档 —— 存档是模型的记忆，工具噪声混进去会把上下文冲掉，导出存档也不该带调试垃圾
