...
import {
  ...
  type Calendar, type TimeUnit,
} from './calendar'
import type { ChatMessage, GameData, LogEntry, TimelineEntry } from '../types/state'
...
export class GameState {
      ...
      if (!raw) {
        ...
        for (const key of LEGACY_KEYS) {
          ...
          if (legacy) {
            ...
            const gs = new GameState(GameState.migrateLegacy(JSON.parse(legacy)))
            gs.save()
            return gs
          }
        }
   ...
   */
  static normalize(saved: unknown): GameData {
   ...
   * 但保留场景、日志、回合数。
   */
  static migrateLegacy(old: unknown): GameData {
    const fresh = createInitialState()
   ...
   *
   * @returns 给模型看的结果文案（成功或失败都返回字符串，不抛异常）
   */
  advanceTime(step: unknown, unit?: unknown, reason = ''): string {
    ...
    const yearsPerUnit: Record<TimeUnit, number> = {
      segment: 4 / 8760, hour: 1 / 8760, day: 1 / 365,
      week: 7 / 365, month: 1 / 12, year: 1,
    }
    if (n * yearsPerUnit[u] > MAX_YEARS) {
      return `⚠ 一次推进跨度太大（${n} ${u}），已忽略。当前：${this.timeLabel}`
    }
    ...
    let result
    try {
      result = cal.advance(this.data.time.iso, n, u)
    } catch (err) {
    ...
    const elapsed = result.elapsedMs < LONG_JUMP_MS ? cal.describeElapsed(result.elapsedMs) : ''
  ...
  setScene(name?: string, description?: string): string {
