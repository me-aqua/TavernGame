...
| # | 位置 | 问题 |
| --- | --- | --- |
| 1 | `core/state.js:283` | 时间线的 `from` 写成了推进**后**的值，导致 `from === to`，起止失去意义。改一行：用推进前的时刻 |
| 2 | `core/calendar.js:101-121` | `describeElapsed` 丢天数：`if (remDays && !years)` 把余数天数静默丢弃（400 天→「1 年1 个月」，370 天→「1 年」）；且拼接缺空格（「1 个月1 天」）。**影响区间 31–182 天**（半年以上不显示这行） |
| 3 | `core/state.js:279` | 时间线记录阈值 `elapsedMs >= 6 小时`，而 `segment` 单位**固定 4 小时** → 默认推进**永远进不了时间线**（侧栏那块基本是空的）。需与 #1 一起修 |

**验证手段**：`.tools/verify-core.mjs`（离线，24 项）+ `.tools/browser.mjs`（CDP 实测）。
bug #1 与 #3 已在真实页面上确认复现。
