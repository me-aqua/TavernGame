...
### 时间引擎的 3 个 bug（2026-09-12 已修，实测通过）

| # | 位置 | 问题 | 修法 |
| --- | --- | --- | --- |
| 1 | `core/state.js` | 时间线的 `from` 写成了推进**后**的值，`from === to`，起止失去意义 | 用推进前的时刻（新增 `beforeIso`） |
| 2 | `core/calendar.js` | `describeElapsed` **每满一年凭空多出 5 天** —— `365 = 12×30 + 5`，`days % 365 / 30` 与 `days % 30` 都从「年」里吃天数（365 天→「1 年 5 天」，730 天→「2 年 10 天」） | 改成**先年 → 再月 → 最后天**逐级剥离；补单位间空格 |
| 3 | `core/state.js` | 时间线门槛「≥ 6 小时」，而 `segment` 固定 4 小时 → 默认推进**永远进不了时间线** | 门槛降到 4 小时 |

> ⚠️ bug #2 起初记成「余数天数被 `!years` 条件丢弃」，实际修的时候才发现
> **根子是两个近似系数打架**：一年按 365 天、一个月按 30 天，而 `365 % 30 = 5`。
> 只删那个条件是不够的 —— 数字仍然对不上。**教训：记录现象时别急着下结论**。

**验证**：`.tools/verify-core.mjs`（离线 35 项）+ `.tools/verify-page.mjs`（真实浏览器 11 项），
两处都全绿才敢提交。后者是这次新写的，补上了「逻辑对 ≠ 页面显示对」这段空白。
