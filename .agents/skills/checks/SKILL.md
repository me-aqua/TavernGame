...
上限是 `SIZE = { file: 550, func: 170, net: 500 }`（`.githooks/pre-commit`）。校准过两次，
两次都是**形状变了**而不是代码变臃肿 —— 但都得在提交信息里说明，别偷偷放宽：

1. Prettier 会拆开模板属性，同语义占更多行（`SettingsDrawer.vue` 201 → 278）。
2. card/3 的格式层是**声明式**的：`card.ts` 522 行里一个顶层键一段校验，
   `card-actions.ts` 437 行是「工具契约 + 执行」两段 —— 行长随格式的键集走，
   不随逻辑复杂度走；拆开只是把同一份格式定义劈成两半（还得给拆出来的模块各配测试）。
   `stores/turn.ts` 的 `createTurnRunner` 162 行是那个闭包工厂本身。
   于是 400 → 550、150 → 170（用户 2026-09-15 拍板「不拆，调上限并说明」）。

**顶到上限时先问「是膨胀还是形状」**：膨胀就拆，形状就把新基线写进这条。
另外 `doc/`、`README`/`AGENTS.md` 与 `cards/*.json` 不受文件行数限制 ——
前两类是文档，卡是作者写的散文（示例卡的原稿逐字迁入），行长由内容决定。
